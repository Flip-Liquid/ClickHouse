...
echo "SELECT 'sha256_success_1'" | ${CLICKHOUSE_CLIENT} --user=multi_double_sha1_user --password=double_sha1_pass_1 2>/dev/null && echo "SUCCESS" || echo "FAILED"
echo "Testing second SHA256 password:"
echo "SELECT 'sha256_success_2'" | ${CLICKHOUSE_CLIENT} --user=multi_double_sha1_user --password=double_sha1_pass_2 2>/dev/null && echo "SUCCESS" || echo "FAILED"
...
echo "Testing mixed auth SHA256 password 1:"
echo "SELECT 'mixed_sha_success_1'" | ${CLICKHOUSE_CLIENT} --user=mixed_auth_user --password=mixed_double_sha1_pass_1 2>/dev/null && echo "SUCCESS" || echo "FAILED"
echo "Testing mixed auth SHA256 password 2:"
echo "SELECT 'mixed_sha_success_2'" | ${CLICKHOUSE_CLIENT} --user=mixed_auth_user --password=mixed_double_sha1_pass_2 2>/dev/null && echo "SUCCESS" || echo "FAILED"
